...
function Gradient_descent_mul_var(X, y, alpha, num_iters, poly_degree, lamda)
...
%      Theta = Theta - alpha * (1 / m ) * (( X * Theta - y )' * X)'; % sin regularizacion
     Theta = Theta * (1 - alpha * (lamda / m)) - alpha * (1 / m ) * (( X * Theta - y )' * X)'; % con regularizacion
%     Theta = pinv(X' * X) * X' * y;
    %disp(Theta);
end
disp(Theta);
